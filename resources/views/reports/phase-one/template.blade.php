...
<style>
    .report-document { margin: -2rem; background: #fff; color: #202020; font-family: Georgia, 'Times New Roman', serif; }
    ...
    .report-brand { letter-spacing: .18em; font: 700 .75rem Arial, sans-serif; color: #6d5a48; }
    .report-cover h1 { margin: 5rem 0 1rem; font: 400 3.2rem/1.1 Georgia, serif; letter-spacing: 0; color: #1d1d1d; }
    .report-cover h2 { margin: 0; font: 400 1.35rem/1.5 Georgia, serif; color: #695c52; }
    .report-cover-name { margin-top: 3rem; font-size: 1.5rem; }
    .report-cover-meta { color: #6f665f; font: .95rem/1.8 Arial, sans-serif; }
    .report-kicker { display: inline-block; padding: .55rem 1.1rem; border: 1px solid #cbbba9; color: #695545; font: 700 .72rem Arial, sans-serif; letter-spacing: .12em; text-transform: uppercase; }
    .report-page h2 { margin: 0 0 2rem; font: 400 2rem/1.2 Georgia, serif; }
    .report-page h3 { margin: 2.5rem 0 1rem; font: 700 1.2rem/1.3 Georgia, serif; }
    ...
    .report-index-number { font: 700 1.5rem Georgia, serif; color: #9d7659; }
    ...
    .report-block-title { display: inline-block; min-width: 52%; padding: .7rem 1.4rem; background: #f8dfcc; box-shadow: 4px 4px 0 rgba(122, 86, 61, .12); font: 700 1.1rem Arial, sans-serif; }
    .report-block-body { padding-top: 1.3rem; }
    .report-table { width: 100%; border-collapse: collapse; margin: 1.5rem 0 2rem; font: .9rem Arial, sans-serif; }
    ...
    .report-note { border-left: 4px solid #b58b67; padding: 1rem 1.2rem; margin: 1.5rem 0; background: #f7f4f0; font: .9rem/1.6 Arial, sans-serif; }
    ...
    .report-pdf-header { position: fixed; top: -10mm; left: 0; right: 0; padding-bottom: 3mm; border-bottom: .4pt solid #d8cabc; color: #8a7768; font: 7.5pt Arial, sans-serif; letter-spacing: .08em; text-align: center; }
    .report-pdf-footer { position: fixed; right: 0; bottom: -10mm; color: #8a7768; font: 7.5pt Arial, sans-serif; }
    .report-pdf-footer::after { content: 'Página ' counter(page); }
    .report-pdf-indicator { display: block; margin: 1.2rem 0 .65rem; color: #795c48; font: 700 .82rem Arial, sans-serif; letter-spacing: .08em; }
    .report-pdf-indicator::before { content: '➜ '; color: #b58b67; }
    .report-arrow-title { display: none; }
...
<div class="report-document">
    ...
    <section class="report-page"><h2>Tu primera lectura</h2><div class="report-gray">@foreach ($report['shared']['intro'] as $paragraph)<p>{!! $paragraph !!}</p>@endforeach</div><div class="report-arrow-title">LAS CUATRO PUERTAS</div><div class="report-pdf-only report-pdf-indicator">LAS CUATRO PUERTAS</div><div class="report-gray">@foreach ($report['shared']['states'] as $paragraph)<p>{!! $paragraph !!}</p>@endforeach</div><div class="report-arrow-title">CONCLUSIONES IMPORTANTES PARA TU LECTURA</div><div class="report-pdf-only report-pdf-indicator">CONCLUSIONES IMPORTANTES PARA TU LECTURA</div><div class="report-gray">@foreach ($report['shared']['conclusions'] as $paragraph)<p>{!! $paragraph !!}</p>@endforeach</div></section>
    <section class="report-page"><div class="report-arrow-title">UNA BREVE INTRODUCCIÓN A TUS CUATRO PUERTAS:</div><div class="report-pdf-only report-pdf-indicator">UNA BREVE INTRODUCCIÓN A TUS CUATRO PUERTAS:</div>@foreach ($report['door_introduction'] as $paragraph)<p>{{ $paragraph }}</p>@endforeach<div class="report-arrow-title">CONCLUSIONES IMPORTANTES PARA TU LECTURA</div><div class="report-pdf-only report-pdf-indicator">CONCLUSIONES IMPORTANTES PARA TU LECTURA</div><div class="report-gray">@foreach ($report['shared']['conclusions'] as $paragraph)<p>{!! $paragraph !!}</p>@endforeach</div><table class="report-table"><tr><th>Puerta</th><th>Posición</th><th>Pregunta</th></tr>@foreach ($report['doors'] as $door)@php $context = $door['blocks']['sign'][0] ?? ''; @endphp<tr><td>{{ $door['title'] }}</td><td>{!! $context !!}</td><td>{{ $door['question'] }}</td></tr>@endforeach</table><div class="report-arrow-title">ESTADOS DE CADA PUERTA</div><div class="report-pdf-only report-pdf-indicator">ESTADOS DE CADA PUERTA</div><div class="report-gray">@foreach ($report['shared']['states'] as $paragraph)<p>{!! $paragraph !!}</p>@endforeach</div></section>
    @foreach ($report['doors'] as $door)<section class="report-page report-door"><div class="report-door-heading"><h2>{{ $door['title'] }}</h2><div class="report-door-subtitle">{{ $door['subtitle'] }}</div></div>@foreach ($door['blocks'] as $block => $paragraphs)@if ($block !== 'shared_intro')<div class="report-block"><div class="report-block-title">{{ $blockTitles[$block] ?? ucfirst(str_replace('_', ' ', $block)) }}</div><div class="report-block-body">@foreach ((array) $paragraphs as $paragraph)<p>{!! $paragraph !!}</p>@endforeach</div></div>@endif @endforeach</section>@endforeach
    <section class="report-page"><h2>Las cuatro puertas juntas</h2>@foreach ($report['combined']['paragraphs'] as $paragraph)<p>{!! $paragraph !!}</p>@endforeach<table class="report-table"><tr><th>Puerta</th><th>Recurso</th><th>Necesidad</th></tr>@foreach ($report['combined']['rows'] as $row)<tr><td>{{ $row['door'] }}</td><td>{{ $row['resource'] }}</td><td>{{ $row['need'] }}</td></tr>@endforeach</table></section>
    <section class="report-page"><h2>Ampliación de tu mapa interior</h2><p>Estas posiciones complementarias amplían la lectura sin introducir todavía aspectos, tránsitos ni predicciones.</p>@foreach ($report['expansion'] as $item)<div class="report-block"><div class="report-block-title">{{ $item['title'] }}</div><div class="report-block-body"><p>{{ $item['content'] }}</p><p>{{ $item['example'] }}</p><p>{{ $item['question'] }}</p></div></div>@endforeach</section>
    <section class="report-page"><h2>Tu práctica de autoobservación</h2><p>{{ $report['practice']['intro'] }}</p><ol>@foreach ($report['practice']['weeks'] as $week)<li>{{ $week }}</li>@endforeach</ol>@foreach ($report['practice']['guidance'] as $paragraph)<p>{{ $paragraph }}</p>@endforeach<h3>Hoja de práctica diaria</h3><p>Fecha ____________________ &nbsp;&nbsp; Fase lunar si deseas anotarla ____________________</p><p>Elige una situación concreta, no una valoración de todo el día.</p><table class="report-table"><tr><th>Campo</th><th>Registro</th></tr><tr><td>La situación</td><td>{{ $report['practice']['sheet'][0] }}<br>____________________________________________________________</td></tr><tr><td>Sol</td><td>{{ $report['practice']['sheet'][1] }}<br>____________________________________________________________</td></tr><tr><td>Luna</td><td>{{ $report['practice']['sheet'][2] }}<br>____________________________________________________________</td></tr><tr><td>Ascendente</td><td>{{ $report['practice']['sheet'][3] }}<br>____________________________________________________________</td></tr><tr><td>Descendente</td><td>{{ $report['practice']['sheet'][4] }}<br>____________________________________________________________</td></tr><tr><td>Mi respuesta</td><td>{{ $report['practice']['sheet'][5] }}<br>____________________________________________________________</td></tr><tr><td>Mi próximo paso</td><td>¿Qué cambio pequeño quiero probar?<br>____________________________________________________________</td></tr></table><p>Carga percibida del 0 al 10 ______ &nbsp;&nbsp; ¿Qué me ayudó? ____________________________</p></section>
