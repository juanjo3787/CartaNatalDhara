...
final class PhaseOnePromptBuilder
{
    ...
    public function build(string $door, array $context): array
    {
        ...
        $stateFormat = 'ESTRUCTURA OBLIGATORIA DE LOS ESTADOS: Cada estado debe contener CUATRO CAPAS SEPARADAS Y COMPLETAS: 1) DESARROLLO INTERPRETATIVO AMPLIO (mínimo 4 párrafos desarrollados antes de cualquier lista) que explique cómo aparece el patrón, qué necesidad existe, qué recurso está actuando, qué sensación puede acompañarlo, qué comportamiento observable puede producir, qué ocurre a corto plazo, qué puede ocurrir después, qué parte aporta el signo, qué parte aporta la casa/eje, qué aporta el regente y cómo interactúan estas piezas en ESTA CARTA CONCRETA. 2) CARACTERÍSTICAS QUE PUEDES OBSERVAR (5-7 características breves, solo características, sin pautas ni ejemplos). 3) PAUTAS Y CONSIDERACIONES (una pauta desarrollada de 40-90 palabras para CADA característica, explicando qué observar, qué distinguir, qué señal reconocer, qué conviene comprobar). 4) EJEMPLOS COTIDIANOS (un ejemplo narrativo de 80-150 palabras para CADA pauta, con contexto, respuesta, experiencia, alternativa y aprendizaje). NUNCA generes CABECERA 1, CABECERA 2, CABECERA 3 seguidas de una única lista. Cada cabecera debe tener SU PROPIO BLOQUE DE CONTENIDO.';
        $system = implode("\n", [
            ...
            'ESTRUCTURA OBLIGATORIA DE HARMONY: 1) DESARROLLO INTERPRETATIVO AMPLIO (mínimo 4 párrafos) que utilice PUERTA + SIGNO + CASA/EJE + REGENTE + SIGNO DEL REGENTE + CASA DEL REGENTE, explicando cómo se reconoce esta combinación concreta cuando tiene medida adecuada. 2) "Características que puedes observar" (5-7 características breves). 3) "Pautas y consideraciones para reconocer este equilibrio" (una pauta desarrollada de 40-90 palabras por característica). 4) "Ejemplos cotidianos de estas pautas" (ejemplos narrativos de 80-150 palabras cada uno).',
            'ESTRUCTURA OBLIGATORIA DE DEFICIT: 1) DESARROLLO INTERPRETATIVO AMPLIO (mínimo 4 párrafos) explicando qué capacidad tiene poco espacio, por qué puede ocurrir, qué intenta proteger, qué alivio inmediato puede proporcionar, qué consecuencia posterior puede generar, cómo intervienen signo, casa y regente. 2) "Características que puedes observar" (7 características específicas). 3) "Pautas y consideraciones para empezar a armonizar" (7 pautas desarrolladas, no órdenes genéricas). 4) "Ejemplos cotidianos y formas de empezar a armonizar" (7 ejemplos narrativos desarrollados).',
            'ESTRUCTURA OBLIGATORIA DE EXCESS: 1) DESARROLLO INTERPRETATIVO AMPLIO (mínimo 4 párrafos) explicando qué capacidad útil está funcionando, cuándo empieza a ocupar demasiado espacio, qué intenta asegurar, por qué cuesta detenerla, qué recompensa inmediato puede mantenerla, qué coste aparece después, cómo intervienen signo, casa y regente. El exceso NO es defecto moral, es un recurso válido que ha perdido medida. 2) "Características que puedes observar" (7 características). 3) "Pautas y consideraciones para recuperar una medida adecuada" (7 pautas desarrolladas). 4) "Ejemplos cotidianos y formas de recuperar medida" (7 ejemplos narrativos).',
