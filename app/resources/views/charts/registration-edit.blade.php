...
<div class="row"><div><label>Fecha de nacimiento<input type="text" name="local_date" data-date-time="date" inputmode="numeric" maxlength="10" placeholder="DD/MM/AAAA" pattern="[0-9]{2}/[0-9]{2}/[0-9]{4}" value="{{ old('local_date', $chart->birthData->local_date->format('d/m/Y')) }}" required></label>@error('local_date')<small class="field-error">{{ $message }}</small>@enderror</div><div><label>Hora de nacimiento<input type="text" name="local_time" data-date-time="time" inputmode="numeric" maxlength="5" placeholder="HH:MM" pattern="[0-9]{2}:[0-9]{2}" value="{{ old('local_time', substr((string) $chart->birthData->local_time, 0, 5)) }}" required></label>@error('local_time')<small class="field-error">{{ $message }}</small>@enderror</div></div>
