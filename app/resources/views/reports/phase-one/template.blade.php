...
<div class="report-document">
    ...
    @foreach ($report['doors'] as $door)<section class="report-page report-door"><div class="report-door-heading"><h1>{{ $door['title'] }}</h1><div class="report-door-subtitle">{{ $door['subtitle'] }}</div></div>@foreach ($door['blocks'] as $block => $paragraphs)@if ($block !== 'shared_intro')<div class="report-block"><h2 class="report-block-title">{{ $blockTitles[$block] ?? ucfirst(str_replace('_', ' ', $block)) }}</h2><div class="report-block-body">@if ($block === 'harmony')<h3>Características que puedes observar</h3><h3>Pautas y consideraciones para reconocer este equilibrio</h3><h3>Ejemplos cotidianos de estas pautas</h3>@elseif ($block === 'deficit')<h3>Características que puedes observar</h3><h3>Pautas y consideraciones para empezar a armonizar</h3><h3>Ejemplos cotidianos y formas de empezar a armonizar</h3>@elseif ($block === 'excess')<h3>Características que puedes observar</h3><h3>Pautas y consideraciones para recuperar una medida adecuada</h3><h3>Ejemplos cotidianos y formas de recuperar medida</h3>@elseif ($block === 'harmonization')<h3>Desde el defecto</h3><h3>Desde el exceso</h3><h3>El punto de equilibrio</h3>@endif @foreach ((array) $paragraphs as $paragraph)@php($trimmedParagraph = trim($paragraph))@if (str_starts_with($trimmedParagraph, '<ol') || str_starts_with($trimmedParagraph, '<ul')){!! $trimmedParagraph !!}@else<p>{!! $paragraph !!}</p>@endif @endforeach</div></div>@endif @endforeach</section>@endforeach
